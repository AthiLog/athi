...
포트란 서버에 들어갈 수 있는 문이 여러개가 있는데 4000이라고 적혀져 있는 문을 열어 놓은거라고 할 수 있다.

`/athi/`는 따로 지정해 놓은것으로 나중에 원하는대로 바꿀 수 있다.

그래서 이제 미리보기 서버를 접속할 수 있다.

인터넷에 아래를 주소창에 입력해주면 접속이 잘된다.
...
.git 파일을 현재 폴더부터 하위에 있는 폴더들에 있는 모든 파일의 변경사항을 추적(기록) 합니다.
